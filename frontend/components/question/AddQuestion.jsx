...
const AddQuestion = () => {
    ...
    const handleSubmit = async(e) =>{
      e.preventDefault()
      try {
        const result = {question, questionType, choices, correctAnswers:correctAnswers.map((answer) =>{
            ...
            <div className='card-body'>
              <form onSubmit={handleSubmit} className='p-2'>
                <div className='mb-3'>
                  ...
                  <select value={subject} id="subject" onChange={(e) => setSubject(e.target.value)} className='form-control'>
                ...
                <div className='mb-3'>
                  ...
                  <select onChange={(e) => setQuestionType(e.target.value)} value={questionType} id="question-type" className='form-control'>
                ...
                <div className='mb-3'>
                  ...
                  {choices.map((choice, index) => (
                    <div key={index} className='input-group mb-3'>
                      ...
                      <button type='button' className='btn btn-outline-danger' onClick={() => handleRemoveChoice(index)}>Remove</button>
                    </div>
                  ))}

                  <button type='button' className='btn btn-outline-primary btn-sm' onClick={handleAddChoice}>Add Choice</button>
                ...
                {questionType === "single" && (
                  <div className='mb-3'>
                     ...
                     <input type="text" value={correctAnswers[0]} onChange={(e) => handleCorrectAnswerChange(0, e.target.value)} className='form-control' />
                  ...
                  <div className='mb-3'>
                     ...
                     {correctAnswers.map((answer, index) => (
                      <div key={index} className='input-group mb-2'>
                        <input type="text" value={answer} onChange={(e) => handleCorrectAnswerChange(index, e.target.value)} className='form-control' />
                        {index > 0 && (
                          <button type='button' className='btn btn-danger btn-sm' onClick={() => handleRemoveCorrectAnswer(index)} >Remove</button>
                        )}
                      </div>
                     ))}
                     <button type='button' className='btn btn-outline-info' onClick={handleAddCorrectAnswer} > Add Correct Answer </button>
                  </div>
                )}
